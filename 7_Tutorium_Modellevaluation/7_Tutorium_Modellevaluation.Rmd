...
\vspace{2mm}
\text{9. Termin: (7) Modellevaluation}
...
knitr::include_graphics("7_Abbildungen/this_is_useless.jpeg")
knitr::include_graphics("7_Abbildungen/after_a_while.jpeg")
```
...
sigsqr_hat  = (1/n)*sum((D$y_i-(beta_0_hat+beta_1_hat*D$x_i))^2) # sigsqu_hat (ML-Schätzer)
sigsqr_hat_unbiased  = (1/(n-1))*sum((D$y_i-(beta_0_hat+beta_1_hat*D$x_i))^2) # sigsqu_hat (unverzerrt)
...
cat("n:", n, "\nStichprobenmittel der x_i_-Werte:", x_bar, "\nStichprobenmittel der y_i-Werte:", 
    ...
    "\nsigsqr_hat_ML-Schätzer:", sigsqr_hat, "\nsigsqr_hat_unverzerrt:", sigsqr_hat_unbiased)
```
...
sigsqu_hat =  (t(y - X %*% beta_hat) %*%           # \frac{(y - X\hat{\beta})^T(y - X\hat{\beta})}{n - p} 
                     (y - X %*% beta_hat))/(n-p)   #      (unverzerrt)
sigsqu_hat_ML =  (t(y - X %*% beta_hat) %*%        # \frac{(y - X\hat{\beta})^T(y - X\hat{\beta})}{n - p + 1}
                     (y - X %*% beta_hat))/(n-p+1) #     (ML-Schätzer) 
# Ausgabe
cat("beta_0_hat:", beta_hat[1] , "\nbeta_1_hat:", beta_hat[2], "\nsigsqr_hat_ML-Schätzer:", 
    sigsqr_hat, "\nsigsqr_hat_unverzerrt:", sigsqr_hat_unbiased)
```
...
# Beispiel T-Statistik - für $n=5$ und $p = 2$
\vspace{3mm}
\setstretch{1}
\color{darkcyan}
\small
...
Wenn wir einen *Kontrastgewichtsvektor* $c = \begin{pmatrix} 1 \\ 0 \end{pmatrix}$, dann fließt $\hat{\beta}_1$ ein-fach in die T-Statistik ein und $\hat{\beta}_2$ gar nicht (null-fach). 
\begin{align*}
T = \frac{\begin{pmatrix}1&0\end{pmatrix}\begin{pmatrix}\hat{\beta}_1\\\hat{\beta}_1\end{pmatrix}}
{\sqrt{\hat{\sigma}^2\begin{pmatrix}1&0\end{pmatrix}
\begin{pmatrix}x_{11}&x_{21}&x_{31}&x_{41}&x_{51}\\x_{12}&x_{22}&x_{32}&x_{42}&x_{52}\end{pmatrix}
\begin{pmatrix}x_{11}&x_{12}\\x_{21}&x_{22}\\x_{31}&x_{32}\\x_{41}&x_{42}\\x_{51}&x_{52}\end{pmatrix}^{-1}
\begin{pmatrix}1\\0\end{pmatrix}}}
\end{align*}
Als ein anderes Beispiel könnten wir auch beide Betaparameterschätzer-Komponenten gleich gewichten, indem wir $c = \begin{pmatrix} \sfrac{1}{2} \\ \sfrac{1}{2} \end{pmatrix}$ wählen. 
...
Einheit (13) Einstichproben-T-Tests in WTFI und Einheit (9) T-Tests in ALM. 
...
\color{darkcyan}
\footnotesize
Im Folgenden sind 3 Beispiele dafür aufgeführt, wie eine T-Teststatistik aussehen könnte, für $p=2$, $n=5$, einen Nummhypothesenbetaparameter $\beta_0 = \begin{pmatrix}0 \\0\end{pmatrix}$ und einen Kontrastgewichtsvektor $c=\begin{pmatrix}1\\0\end{pmatrix}$. Bitte beachte, dass jedem Beispiel ein anderer Datensatz, i.e. $y^{(1)}, y^{(2)}$ und $y^{(3)}$  zugrunde liegt, mit welchem die entsprechenden Betaparameterschätzer, i.e. $\hat{\beta}^{(1)}, \hat{\beta}^{(2)}$ und $\hat{\beta}^{(3)}$, bestimmt wurden (Sonst hätten die $\hat{\beta}$s in allen 3 Beispielen den gleichen Wert).
...
Beispiel 1) Wir haben mithilfe eines Datensatzes $y^{(1)}$ den Betaparameter geschätzt und $\hat{\beta}^{(1)}=\begin{pmatrix}0\\0\end{pmatrix}$ erhalten.

\tiny
\begin{align*}
T^{(1)} = \frac{\begin{pmatrix}1&0\end{pmatrix}\begin{pmatrix}0\\0\end{pmatrix}- 
\begin{pmatrix}1&0\end{pmatrix}\begin{pmatrix}0\\0\end{pmatrix}}
{\sqrt{\hat{\sigma}^2 c^T(X^TX)^{-1}c}}
= \frac{0-0}{\sqrt{\hat{\sigma}^2 c^T(X^TX)^{-1}c}}
= 0
\end{align*}

\footnotesize
Beispiel 2) Wir haben mithilfe eines Datensatzes $y^{(2)}$ den Betaparameter geschätzt und $\hat{\beta}^{(2)}=\begin{pmatrix}2.81\\0.44\end{pmatrix}$ erhalten.
\tiny
\begin{align*}
T^{(2)} = \frac{\begin{pmatrix}1&0\end{pmatrix}\begin{pmatrix}2.81\\0.44\end{pmatrix}- 
\begin{pmatrix}1&0\end{pmatrix}\begin{pmatrix}0\\0\end{pmatrix}}
{\sqrt{\hat{\sigma}^2 c^T(X^TX)^{-1}c}}
= \frac{2.81-0}{\sqrt{\hat{\sigma}^2 c^T(X^TX)^{-1}c}} \neq 0
\end{align*}

\footnotesize
Beispiel 3) Wir haben mithilfe eines Datensatzes $y^{(3)}$ den Betaparameter geschätzt und $\hat{\beta}^{(3)}=\begin{pmatrix}-4.33\\0.03\end{pmatrix}$ erhalten.
\tiny
\begin{align*}
T^{(3)} = \frac{\begin{pmatrix}1&0\end{pmatrix}\begin{pmatrix}-4.33\\0.03\end{pmatrix}- 
\begin{pmatrix}1&0\end{pmatrix}\begin{pmatrix}0\\0\end{pmatrix}}
{\sqrt{\hat{\sigma}^2 c^T(X^TX)^{-1}c}}
= \frac{-4.33-0}{\sqrt{\hat{\sigma}^2 c^T(X^TX)^{-1}c}} \neq 0
\end{align*}

Anmerkung:

* \color{darkcyan} Zu Beispiel 1) Es ist äußerst unwahrscheinlch (idR unmöglich) ein $\hat{\beta}=\begin{pmatrix}0\\0\end{pmatrix}$  zu schätzen. Dieses Beispiel ist hier nur dafür da, um veranschaulichen, wie die T-Teststatistik bei unterschiedlichen Beispieldatensätzen und entsprechenden unterschiedlichen $\hat{\beta}$s aussehen kann. 
...
* Der Term $c^T\hat{\beta} - c^T\beta_0$ misst den Abstand zwischen einem erechneten $\hat{\beta}$ und dem Nullhypothesenbetaparameter $\beta_0$
* $T=0$, für $c^T\hat{\beta}  = c^T\beta_0$
* $T\neq0$, für $c^T\hat{\beta}  \neq c^T\beta_0$
...
Simulation Unabhängig und identische normalverteilte Zufallsvariablen (aus VO)
...
# Speichern
dev.copy2pdf(
file        = file.path("7_Abbildungen", "alm_7_T_Teststatistik_1.pdf"),
width       = 8,
height      = 4)
```
...
* \color{darkcyan} $\hat{\varepsilon}_1$ in \eqref{part_1} ist hier nicht das gleiche wie  $\hat{\varepsilon}_1$ \eqref{voll}!
* in \eqref{part_1} bezeichnet $\hat{\varepsilon}_1 \in \mathbb{R}^n$ den Residualvektor, der aus der 1. Partitionierung resultiert und in \eqref{voll} bezeichnet 
* in \eqref{voll} bereichnet $\hat{\varepsilon}_1 \in \mathbb{R}^1$ einen Eintrag des Residualvektors der aus dem vollständigen Modell resultiert. 
...
\hat{\varepsilon}_1 := y - X_1\hat{\beta}_1
\end{equation}
die Residuenvektoren des vollständigen und des reduzierten Modells, respektive.
Dann gilt
\begin{equation}
\ln \qoppa =
...
* Die F-Statistik und der Likelihood-Quotient sind nicht das gleiche, messen aber intuitiv das gleiche. 
...
* Anders ausgedrückt messen beide Größen, inwieweit das Hinzunehmen der $p_2$ Regressoren (die im vollständigen Modell beinhaltet sind) die Erklärung der Daten im Vergleich zum reduzierten Modell (das keine $p_2$ Regressoren beinhaltet) verbessert.
...
\footnotesize
\color{darkcyan}
Visualisierung zweier beispielhafter wahrer, aber unbekannten Betaparameter. 
...
p_1    = 1                                                # Anzahl Betaparameter reduziertes Modell
p_2    = 1                                                # Anzahl zusätzlicher Betaparameter vollständiges Modell
p      = p_1 + p_2                                        # Anzahl Betaparameter im vollständigem Modell
x      = 1:n                                              # Prädiktorwerte
X      = matrix(c(rep(1,n),x), nrow = n)                  # Designmatrix des vollständigen Modells
X_1    = X[,1]                                            # Designmatrix des reduzierten Modells
I_n    = diag(n)                                          # n x n Einheitsmatrix
...
for(s in 1:nscn){                                         # Szenarieniterationen
  ...
  eps_1_hat       = y-X_1%*%beta_hat_1                    # Residuenvektor reduziertes Modell
  eps_hat         = y-X%*%beta_hat                        # Residuenvektor vollständiges Modell
  eps_1_eps_1_hat = t(eps_1_hat) %*% eps_1_hat            # RQS reduziertes Modell
  eps_eps_hat     = t(eps_hat)   %*% eps_hat              # RQS vollständiges Modell
  Eff[s]          = (((eps_1_eps_1_hat-eps_eps_hat)/p_2)/ # F-Statistik
                       (eps_eps_hat/(n-p)))}
```

```{r, echo = F, eval=F}
cat("F-Statistik für beta_2  = 0_{p_2}:", Eff[1],
    "\nF-Statistik für beta_2 != 0_{p_2}:", Eff[2])
```
...
n      = 10                                                 # Anzahl Datenpunkte
p      = 2                                                  # Anzahl Betaparameter
p_1    = 1                                                  # Anzahl Betaparameter im reduzierten Modell
p_2    = 1                                                  # Anzahl zusätzlicher Betaparameter im vollständigen Modell
p      = p_1 + p_2                                          # Anzahl Betaparameter im vollständigem Modell
x      = 1:n                                                # Prädiktorwerte
X      = matrix(c(rep(1,n),x), nrow = n)                    # Designmatrix des vollständigen Modells
X_1    = X[,1]                                              # Designmatrix des reduzierten Modells
...
for(s in 1:nscn){                                           # Szenarieniterationen
  delta[s] = (t(t(K)%*%beta[,s])%*%                         # Nichtzentralitätsparameter
  ...
  for(i in 1:nsim){                                         # Simulationsiterationen
    ...
    eps_1_eps_1_hat = t(eps_1_hat) %*% eps_1_hat            # RQS reduziertes Modell
    eps_eps_hat     = t(eps_hat)   %*% eps_hat              # RQS vollständiges Modell
    Eff[s,i]        = (((eps_1_eps_1_hat-eps_eps_hat)/p_2)/ # F-Statistik
                         (eps_eps_hat/(n-p)))
  }
}
```
...
xlimits     = list(c(.05,6), c(0,80))
ylimits     = list(c(0,2), c(0,0.05))
breaks      = c(600,200)
...
# Speichern
dev.copy2pdf(
file        = file.path("7_Abbildungen", "alm_7_F_Statistik_Tut.pdf"),
width       = 8,
height      = 4)
```
\vfill
```{r, echo = FALSE, out.width = "100%"}
knitr::include_graphics("7_Abbildungen/alm_7_F_Statistik_Tut.pdf")
```
\vfill
