...
\vspace{2mm}
\text{8. Termin (Teil 1): Modellschätzungen}
...
* Für diese probabilistischen Prozesse gehen wir davon aus, dass ihnen bestimmte Verteilungen bzw. Wahrscheinlichkeitsdichten zugrundeliegen (z.B. Normalverteilung)
* Wir verwenden erhobene Daten dafür, Parameterwerte zu schätzen. 
* Dabei bilden die angenommenen Verteilungen bzw. Warhscheinlihckeitsdichten der probabilistischen Prozesse, die die Daten (so nehmen wir an) generiert haben, die Grundlage für die Parameterschätzung bzw. der Herleitung der Formeln, die wir dafür verwenden.  
...
\begin{align*}
y = X \beta + \epsilon \Leftrightarrow \begin{pmatrix}y_1\\y_2\\y_3\\y_4\\y_5\end{pmatrix} 
...
\footnotesize
\vspace{2mm}
Dann sieht Betaparameterschätzerformel ausgeschrieben wie folgt aus
...
\begin{pmatrix}y_1\\y_2\\y_3\\y_4\\y_5\end{pmatrix} \\
=& \hspace{2mm} \dots \\
=& \hspace{2mm}\hat{\beta} = \begin{pmatrix}\hat{\beta_1}\\\hat{\beta_2}\end{pmatrix} \in \mathbb{R}^p
\end{align*}
...
* Somit ergibt sich insgesamt für $\hat{\beta}$ die Größe $(2\times2)(2\times1)=(2\times1)$. 
...
\tiny
\begin{align*}
y_i = \mu + \epsilon_i, \epsilon_i \sim N(0,\sigma^2) \mbox{ für } i = 1,...,n, \text{ mit unabhängigen }\epsilon_i
\end{align*}
...
\beta + \begin{pmatrix}\epsilon_1 \\ \vdots \\ \epsilon_n \end{pmatrix}
= \begin{pmatrix}1\\ \vdots \\1\end{pmatrix}
\mu + \begin{pmatrix}\epsilon_1 \\ \vdots \\ \epsilon_n \end{pmatrix}
= \begin{pmatrix}\mu+\epsilon_1\\ \vdots \\ \mu+\epsilon_n\end{pmatrix} 
\end{align*}
...
* Das Bestimmen des Betaparameterschätzers $\hat{\beta}$ mit ```mean(y)``` und des Varianzparameterschätzers $\hat{\sigma}^2$ mit ```var(y)``` wird hier nur angeführt, um zu zeigen, dass es in diesem Szenario das gleiche ergibt wie über die allgemeine ALM-Formeln $(X^T)X^{-1}X^Ty$ und $\frac{(y - X\hat{\beta})^T(y - X\hat{\beta})}{n - p}$.
